  ...
  "Statement": [
    {
      ...
      "Effect": "Allow",
      "Principal": {
        "AWS": "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/trading_server_instance_role_${var.environment}"
      },
...
resource "aws_iam_role" "instance_role" {
  name = "trading_server_instance_role_${var.environment}"
...
resource "aws_iam_role_policy" "s3_list_policy" {
  name = "s3-list-access-${var.environment}"
  role = aws_iam_role.instance_role.id
...
resource "aws_iam_instance_profile" "instance_profile" {
  name = "trading_server_instance_profile_${var.environment}"
  role = aws_iam_role.instance_role.name
}
...
resource "aws_instance" "model_instance" {
  ...
  tags = {
    Name        = "trading-server-${var.environment}-${each.value.symbol}-${each.value.model_type}-${each.value.model_number}"
